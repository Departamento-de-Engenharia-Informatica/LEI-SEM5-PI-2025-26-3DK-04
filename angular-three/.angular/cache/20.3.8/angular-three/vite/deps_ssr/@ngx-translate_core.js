...
var TranslateStore = class _TranslateStore {
  _onTranslationChange = new import_rxjs.Subject();
  _onLangChange = new import_rxjs.Subject();
  _onFallbackLangChange = new import_rxjs.Subject();
  fallbackLang = null;
  currentLang;
  translations = {};
  languages = [];
...
var TranslateService = class _TranslateService {
  ...
  setFallbackLang(lang) {
    ...
    this.store.setFallbackLang(lang);
    return (0, import_rxjs.of)(this.store.getTranslations(lang));
  }
  /**
  ...
  get(key, interpolateParams) {
    ...
    if (this.pending) {
      return this.loadingTranslations.pipe((0, import_operators.concatMap)(() => {
  ...
  getStreamOnTranslationChange(key, interpolateParams) {
    ...
    return (0, import_rxjs.concat)((0, import_rxjs.defer)(() => this.get(key, interpolateParams)), this.onTranslationChange.pipe((0, import_operators.switchMap)(() => {
  ...
  stream(key, interpolateParams) {
    ...
    return (0, import_rxjs.concat)((0, import_rxjs.defer)(() => this.get(key, interpolateParams)), this.onLangChange.pipe((0, import_operators.switchMap)(() => {
      const res = this.getParsedResult(key, interpolateParams);
      return makeObservable(res);
    })));
  }
