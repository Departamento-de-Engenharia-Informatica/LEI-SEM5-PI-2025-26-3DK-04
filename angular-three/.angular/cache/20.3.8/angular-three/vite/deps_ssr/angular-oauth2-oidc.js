...
import {
  CommonModule
} from "./chunk-OVRRYMOE.js";
import {
  HTTP_INTERCEPTORS,
  HttpClient,
  HttpHeaders,
  HttpParams
} from "./chunk-JQEFFM6J.js";
...
var AbstractValidationHandler = class {
  ...
  async validateAtHash(params) {
    ...
    const tokenHash = await this.calcHash(params.accessToken, hashAlg);
...
var _OAuthService = class _OAuthService extends AuthConfig {
  constructor(ngZone, http, storage, tokenValidationHandler, config, urlHelper, logger, crypto, document, dateTimeService) {
    ...
    this.state = "";
    this.eventsSubject = new import_rxjs.Subject();
    this.discoveryDocumentLoadedSubject = new import_rxjs.Subject();
  ...
  setupSessionCheck() {
    this.events.pipe((0, import_operators.filter)((e) => e.type === "token_received")).subscribe(() => {
      this.initSessionCheck();
    });
  }
  /**
  ...
  setupAutomaticSilentRefresh(params = {}, listenTo, noPrompt = true) {
    ...
    this.automaticRefreshSubscription = this.events.pipe((0, import_operators.tap)((e) => {
      if (e.type === "token_received") {
        shouldRunSilentRefresh = true;
      } else if (e.type === "logout") {
        shouldRunSilentRefresh = false;
      }
    }), (0, import_operators.filter)((e) => e.type === "token_expires" && (listenTo == null || listenTo === "any" || e.info === listenTo)), (0, import_operators.debounceTime)(1e3)).subscribe(() => {
  ...
  setupRefreshTimer() {
    ...
    this.tokenReceivedSubscription = this.events.pipe((0, import_operators.filter)((e) => e.type === "token_received")).subscribe(() => {
      this.clearAccessTokenTimer();
      this.clearIdTokenTimer();
      this.setupExpirationTimers();
    });
  ...
  setupAccessTokenTimer() {
    ...
    this.ngZone.runOutsideAngular(() => {
      this.accessTokenTimeoutSubscription = (0, import_rxjs.of)(new OAuthInfoEvent("token_expires", "access_token")).pipe((0, import_operators.delay)(timeout2)).subscribe((e) => {
        this.ngZone.run(() => {
  ...
  setupIdTokenTimer() {
    ...
    const timeout2 = this.calcTimeout(storedAt, expiration);
    this.ngZone.runOutsideAngular(() => {
      this.idTokenTimeoutSubscription = (0, import_rxjs.of)(new OAuthInfoEvent("token_expires", "id_token")).pipe((0, import_operators.delay)(timeout2)).subscribe((e) => {
        this.ngZone.run(() => {
          this.eventsSubject.next(e);
        });
      });
  ...
  refreshToken() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.post(this.tokenEndpoint, params, {
        headers
      }).pipe((0, import_operators.switchMap)((tokenResponse) => {
        if (this.oidc && tokenResponse.id_token) {
          return (0, import_rxjs.from)(this.processIdToken(tokenResponse.id_token, tokenResponse.access_token, true)).pipe((0, import_operators.tap)((result) => this.storeIdToken(result)), (0, import_operators.map)(() => tokenResponse));
        } else {
          return (0, import_rxjs.of)(tokenResponse);
        }
      })).subscribe((tokenResponse) => {
  ...
  silentRefresh(params = {}, noPrompt = true) {
    ...
    const errors = this.events.pipe((0, import_operators.filter)((e) => e instanceof OAuthErrorEvent), (0, import_operators.first)());
    const success = this.events.pipe((0, import_operators.filter)((e) => e.type === "token_received"), (0, import_operators.first)());
    const timeout2 = (0, import_rxjs.of)(new OAuthErrorEvent("silent_refresh_timeout", null)).pipe((0, import_operators.delay)(this.silentRefreshTimeout));
    return (0, import_rxjs.race)([errors, success, timeout2]).pipe((0, import_operators.map)((e) => {
  ...
  waitForSilentRefreshAfterSessionChange() {
    this.events.pipe((0, import_operators.filter)((e) => e.type === "silently_refreshed" || e.type === "silent_refresh_timeout" || e.type === "silent_refresh_error"), (0, import_operators.first)()).subscribe((e) => {
  ...
  initImplicitFlow(additionalState = "", params = "") {
    if (this.loginUrl !== "") {
      ...
      this.events.pipe((0, import_operators.filter)((e) => e.type === "discovery_document_loaded")).subscribe(() => this.initImplicitFlowInternal(additionalState, params));
    }
  }
  /**
  ...
  initCodeFlow(additionalState = "", params = {}) {
    if (this.loginUrl !== "") {
      ...
      this.events.pipe((0, import_operators.filter)((e) => e.type === "discovery_document_loaded")).subscribe(() => this.initCodeFlowInternal(additionalState, params));
    }
  }
  ...
  revokeTokenAndLogout(customParameters = {}, ignoreCorsIssues = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (accessToken) {
        ...
        revokeAccessToken = (0, import_rxjs.of)(null);
      }
      if (refreshToken) {
        ...
        revokeRefreshToken = (0, import_rxjs.of)(null);
      }
      if (ignoreCorsIssues) {
        revokeAccessToken = revokeAccessToken.pipe((0, import_operators.catchError)((err2) => {
          if (err2.status === 0) {
            return (0, import_rxjs.of)(null);
          }
          return (0, import_rxjs.throwError)(err2);
        }));
        revokeRefreshToken = revokeRefreshToken.pipe((0, import_operators.catchError)((err2) => {
          if (err2.status === 0) {
            return (0, import_rxjs.of)(null);
          }
          return (0, import_rxjs.throwError)(err2);
      ...
      (0, import_rxjs.combineLatest)([revokeAccessToken, revokeRefreshToken]).subscribe((res) => {
...
var _DefaultOAuthInterceptor = class _DefaultOAuthInterceptor {
  ...
  intercept(req, next) {
    ...
    if (!sendAccessToken) {
      return next.handle(req).pipe((0, import_operators.catchError)((err2) => this.errorHandler.handleError(err2)));
    }
    return (0, import_rxjs.merge)((0, import_rxjs.of)(this.oAuthService.getAccessToken()).pipe((0, import_operators.filter)((token) => !!token)), this.oAuthService.events.pipe(
      (0, import_operators.filter)((e) => e.type === "token_received"),
      (0, import_operators.timeout)(this.oAuthService.waitForTokenInMsec || 0),
      (0, import_operators.catchError)(() => (0, import_rxjs.of)(null)),
      // timeout is not an error
      (0, import_operators.map)(() => this.oAuthService.getAccessToken())
    )).pipe((0, import_operators.take)(1), (0, import_operators.mergeMap)((token) => {
      if (token) {
