...
% ============================================
% This algorithm prioritizes vessels with the minimum slack time,
% using a greedy approach for computational efficiency.
% Slack time = (Departure - Arrival) - (Unload + Load)
% Declaração dinâmica para permitir assertz/retract em runtime
:- dynamic vessel/6.
:- dynamic ignored_notification/2.

% ============================================
% HEURISTIC ALGORITHM - MINIMUM SLACK TIME
% ============================================

% heuristic_minimum_slack_time(SeqTripletsH, SDelaysH).
% Returns the sequence of vessels with delays calculated using MST heuristic
...
heuristic_minimum_slack_time(SeqTripletsH, SDelaysH) :-
    ...
    format(user_error, '[INFO] MST Heuristic - Found ~w unique docks: ~w~n', [length(UniqueDocks), UniqueDocks]),
    % For each dock, calculate the best schedule using MST heuristic
    obtain_mst_schedule_per_dock(UniqueDocks, AllSchedules, TotalDelay),
    % Combine all schedules
    flatten(AllSchedules, SeqTripletsH),
    SDelaysH = TotalDelay.

% obtain_mst_schedule_per_dock(+Docks, -AllSchedules, -TotalDelay)
% For each dock, finds the schedule using MST heuristic and sums delays
obtain_mst_schedule_per_dock([], [], 0).
obtain_mst_schedule_per_dock([DockId|RestDocks], [Schedule|RestSchedules], TotalDelay) :-
    format(user_error, '[INFO] MST Heuristic - Processing dock: ~w~n', [DockId]),
    findall((SlackTime, V), (vessel(V, DockId, Arrival, Departure, Unload, Load), SlackTime is (Departure - Arrival) - (Unload + Load)), LSTV),
    sort(LSTV, LSTVSorted),
    obtain_vessels(LSTVSorted, SeqV),
    sequence_temporization(SeqV, Schedule),
    sum_delays(Schedule, DockDelay),
    format(user_error, '[INFO] MST Heuristic - Dock ~w has ~w vessels~n', [DockId, length(SeqV)]),
    format(user_error, '[INFO] MST Heuristic - Dock ~w: delay = ~w~n', [DockId, DockDelay]),
    obtain_mst_schedule_per_dock(RestDocks, RestSchedules, RestDelay),
    TotalDelay is DockDelay + RestDelay.

% obtain_vessels(+LSTVSorted, -SeqV)
% Extracts vessel IDs from sorted list of (SlackTime, Vessel) pairs
obtain_vessels([], []).
obtain_vessels([(_, V)|LSTV], [V|LV]) :-
    obtain_vessels(LSTV, LV).
