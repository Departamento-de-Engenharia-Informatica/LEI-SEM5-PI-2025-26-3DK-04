...
@Component({
  selector: 'app-manage-vessel-visit-notifications-rep',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './manageVesselVisitNotificationsRep.html',
  styleUrls: ['./manageVesselVisitNotificationsRep.scss']
})
export class ManageVesselVisitNotificationsRep implements OnInit {

  // Listas de Notificações
  notifications: any[] = [];
  inProgressNotifications: any[] = [];
  // REMOVIDA: submittedNotifications: any[] = [];
  withdrawnNotifications: any[] = [];
  ...
  // REMOVIDO: Review modal state (Foi para SubmittedVesselVisitNotifications)
  // showReviewModal: boolean = false;
  // reviewAction: 'approve' | 'reject' | null = null;
  // reviewNotification: any = null;
  // reviewForm: any = { dockId: null, officerId: '', reason: '' };
  ...
  loadAllNotifications() {
    ...
    // REMOVIDO: this.loadSubmittedNotifications();
    this.loadWithdrawnNotifications();
  }
  ...
  // REMOVIDO: loadSubmittedNotifications()

  // Carregar notificações com status Withdrawn
  ...
  submitNotification(notification: any) {
    ...
    this.adminService.submitVesselVisitNotification(notification.id).subscribe({
      next: () => {
        ...
        this.loadAllNotifications(); // Recarrega InProgress e Withdrawn
        // Necessário recarregar o novo componente Submitted se estiver na mesma página
      },
  ...
  withdrawNotification(notification: any) {
    ...
    this.adminService.withdrawVesselVisitNotification(notification.id).subscribe({
      next: () => {
        ...
        this.loadAllNotifications(); // Recarrega InProgress e Withdrawn
        // Necessário recarregar o novo componente Submitted se a notificação estiver lá
      },
  ...
  /* -------------------
       REVIEW (APPROVE/REJECT) - REMOVIDO (Foi para o novo componente)
  ------------------- */
  // openReviewModal() { ... }
  // closeReviewModal() { ... }
  // confirmReview() { ... }
